...
export default function CreatorLogin() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("collabry.creator.rememberedHandle");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!handle.trim() || !password) { setError("Enter your Instagram handle and password"); return; }
    setSubmitting(true); setError("");
    try {
      const cleanHandle = handle.trim().replace(/^@/, "");
      const r = await fetch(`${BASE_URL}/api/auth/creator/login`, {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ instagramHandle: cleanHandle, password, rememberMe }),
      });
      ...
      try {
        if (rememberMe) localStorage.setItem("collabry.creator.rememberedHandle", cleanHandle);
    ...
    <div className="min-h-screen" style={{ background: "#0A0A0F", fontFamily: POPPINS }}>
      <header className="px-6 py-4 flex items-center justify-between">
        ...
        <Link href="/signup-brand">
          <button className="border border-white text-white text-[11px] px-4 py-2 rounded-full hover:bg-white/10 transition-colors">Signup / Login as Brand</button>
        </Link>
      </header>
      ...
      <div className="flex justify-center px-6 py-8 pb-16">
        <div className="w-full max-w-[480px] rounded-2xl p-6 lg:p-8" style={CARD_STYLE}>
          ...
          <div className="flex rounded-full mb-6 overflow-hidden" style={{ background: "rgba(255,255,255,0.08)" }}>
            <Link href="/signup-creator" className="flex-1">
          ...
          {isBanned && (
            <div className="mb-4 p-4 rounded-xl text-center" style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.25)" }}>
              <p className="text-sm font-semibold text-red-400 mb-1" style={{ fontFamily: POPPINS }}>Your Collabry account is banned.</p>
          ...
          {!isBanned && error && <div className="mb-4 p-3 rounded-lg bg-red-500/20 border border-red-500/30 text-red-400 text-xs">{error}</div>}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input className={inputClass + " pl-8"} type="text" name="username" placeholder="yourhandle" value={handle}
                  onChange={e => {
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer" onClick={() => setRememberMe(v => !v)}>
                <div className="w-4 h-4 rounded border flex items-center justify-center"
                  style={{ background: rememberMe ? "#E14F69" : "transparent", borderColor: rememberMe ? "#E14F69" : "rgba(255,255,255,0.20)" }}>
              ...
              <Link href="/forgot-password-creator" className="text-[#E14F69] text-xs hover:underline">Forgot Password?</Link>
            </div>
          ...
          <p className="text-center text-white/70 text-xs mt-6">
            Don't have an account? <Link href="/signup-creator" className="text-[#E14F69] hover:underline">Sign up as a Creator</Link>
          </p>
        </div>
      </div>
    </div>
